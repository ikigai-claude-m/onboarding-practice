...
export default class WinnerView extends PopUpView {
  ...
  onDraw(width: number, height: number) {
    console.log("WinnerView", width, height);
  }

  createBackground() {
    this.spriteBg = this.createSprite(
      this.backgroundId,
    ...
    this.spriteBg.position.set(
      uiSetting.gameWidth / 2,
      uiSetting.gameHeight / 2
    );
  }
  ...
  private createTitle() {
    ...
    let ropePoints =
      maxRopePoints -
      Math.round((sprite.texture.width / (radius * Math.PI)) * maxRopePoints);
    ropePoints /= 2.7;
    const points = [];
  ...
  private createCanvasTitleText(text: string, fontSize: number) {
    const setting = this.getCanvasSetting({
      fontSize: fontSize,
      letterSpacing: "4.654px",
    });
    const { canvas, ctx } = GradientTextTexture.createOriginCanvas(
      text,
      setting
    );
    ...
    // top
    const { canvas: topCanvas, ctx: topCtx } = GradientTextTexture.copyCanvas(
      canvas,
      ctx!
    );
    ...
    const topFillGradient = GradientTextTexture.createGradient(
      topCtx!,
      topCanvas.height,
      topFillGradientSetting
    );
    const topStrokeGradient = GradientTextTexture.createGradient(
      topCtx!,
      topCanvas.height,
      topStrokeGradientSetting
    );
    GradientTextTexture.strokeText(
      topCtx!,
      topStrokeGradient,
      text,
      centerX,
    ...
    // stroke
    const { canvas: strokeCanvas, ctx: strokeCtx } =
      GradientTextTexture.copyCanvas(canvas, ctx!);
    const strokeGradientSetting = [
      { position: 0.04, color: "#9A2203" },
      { position: 1, color: "#9A2203" },
    ];
    const strokeGradient = GradientTextTexture.createGradient(
      topCtx!,
      topCanvas.height,
      strokeGradientSetting
    );
    GradientTextTexture.strokeText(
      strokeCtx!,
      strokeGradient,
    ...
    // inner
    const { canvas: innerCanvas, ctx: innerCtx } =
      GradientTextTexture.copyCanvas(canvas, ctx!);
    GradientTextTexture.fillText(innerCtx!, text, centerX, centerY, "#D93A14");

    const { canvas: tempCanvas, ctx: tempCtx } = GradientTextTexture.copyCanvas(
      canvas,
      ctx!
    );
    const fillGradient = [{ position: 0.89, color: "#992102" }];
    const fGradient = GradientTextTexture.createGradient(
      tempCtx!,
      topCanvas.height,
      fillGradient
    );
  ...
  private createCanvasSubTitleText(text: string, fontSize: number) {
    ...
    const textGradient = GradientTextTexture.createGradient(
      ctx,
      canvas.height,
  ...
  protected addGradientText(
    ...
    const sprite = this.createGradientText(
      text,
      posX,
      posY,
      fontSize,
